...
refactoring-cli vendors its `.claude/skills/` from **guildmaster** — the
...
Every vendored `SKILL.md` carries `type: command`. refactoring-cli
...
#   - consumer-identifying prose: `guildmaster` → `refactoring-cli` (NOT
...
If a re-sync would lose a refactoring-cli adaptation, lift the change
...
#     → `which refactoring-cli vendors from guildmaster`
