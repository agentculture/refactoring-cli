...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope refactoring-cli
...
scope:** the default personal scope is **private** (`--scope refactoring-cli
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `refactoring-cli`). Private records are served only to a recall in the same
